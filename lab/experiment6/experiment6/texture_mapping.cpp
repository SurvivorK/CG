...
void TextureMapping::initBlendShader() {
	...
	const char* fragCode =
		...
		"	vec4 mixcolor =  mix(texture(mapKd1, TexCoord), texture(mapKd2, TexCoord), material.blend);\n"
		"	color = vec4(ambient + diffuse, 1.0f) * mixcolor;\n"
		"}\n";
	//----------------------------------------------------------------
...
void TextureMapping::initCheckerShader() {
	...
	const char* fragCode =
		...
		"void main() {\n"
		"	int x = int(TexCoord.x * material.repeat) % 2;\n"
		"	int y = int(TexCoord.y * material.repeat) % 2;\n"
		//"	int temp = int((TexCoord.x + TexCoord.y) * material.repeat) % 2;\n"
		"	color = vec4((1 - (x + y) % 2) * material.color1 + (x + y) % 2 * material.color2, 1.0); \n"
		//"	color = vec4(temp == 0 ? material.color1 : material.color2, 1.0); \n"
		"}\n";
	//----------------------------------------------------------------
...
void TextureMapping::handleInput() {
	const float angluarVelocity = 0.5f;
